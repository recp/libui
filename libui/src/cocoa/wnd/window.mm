...
wnd::Window::WindowImpl::WindowImpl(Window *_self, Rect rect, int style) : m_self(_self) {
  NSInteger windowMask = translateWndStyleToCocoaWS(style);

  NSRect screenRect = [[NSScreen mainScreen] visibleFrame];

  NSRect consideredWndRect = [NSWindow frameRectForContentRect: NSMakeRect(0, 0, rect.size.width, rect.size.height)
                                                                                                              styleMask: windowMask];

  double originY = screenRect.size.height - screenRect.origin.y - consideredWndRect.size.height - rect.origin.y;
...
void wnd::Window::WindowImpl::setTitle(const char *title) const {
  [m_wnd setTitle: [NSString stringWithCString: title encoding: NSUTF8StringEncoding]];
}
